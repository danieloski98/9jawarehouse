    ...
    <Modal isOpen={open} onClose={() => close()} isCentered size="xl" closeOnEsc={false} closeOnOverlayClick={false}>
        ...
        <ModalContent>
            <ModalBody className='w-full h-full flex justify-center items-center flex-col py-10'>
                    ...
                    <p className='font-light mt-10'>Your password has been updated!</p>
                    <Link href="/auth/loginform" passHref >
                        <button className="xl:w-4/6 lg:w-4/6 md:w-full sm:w-full mt-4 h-12 bg-themeGreen cursor-pointer flex justify-center items-center mb-10">
                            <span className="ml-4 text-sm font-semibold text-white">Proceed to Login</span>
                        </button>
                    </Link>
            </ModalBody>
        </ModalContent>
    ...
    return (
        ...
        <div className="flex-1 flex flex-col">
            ...
            <div className="flex-1 xl:pl-10 lg:pl-10 md:pl-5 sm:pl-5 xl:pr-0 lg:pr-0 md:pr-5 sm:pr-5 flex flex-col justify-center items-start">
                <h3 className="text-3xl font-semibold text-gray-700">Reset Your Password</h3>
                <p className="text-sm font-light mt-4 text-gray-500">Lets help you create a new secure password</p>

                <div className="flex flex-col xl:w-4/6 lg:w-4/6 md:w-full sm:w-full mt-6">
                    <label className="font-light text-sm">New Password</label>
                    <InputGroup bgColor="#F1EEEE" className="mt-2" size="lg">
                        <InputLeftElement>
                            <FiLock size={25} color="gray" />
                        </InputLeftElement>
                        <Input type="password" name="newpassword" fontSize="sm" value={formik.values.newpassword} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                    </InputGroup>
                    {formik.touched.newpassword && formik.errors.newpassword && (
                        <p className="text-red-500 font-semibold text-xs mt-1">{formik.errors.newpassword}</p>
                    )}
                </div>
                ...
                <div className="flex flex-col xl:w-4/6 lg:w-4/6 md:w-full sm:w-full mt-6">
                    ...
                    <InputGroup bgColor="#F1EEEE" className="mt-2" size="lg">
                        ...
                        <Input type="password" name="confirmpassword" fontSize="sm" value={formik.values.confirmpassword} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                    </InputGroup>
                    {formik.touched.confirmpassword && formik.errors.confirmpassword && (
                        <p className="text-red-500 font-semibold text-xs mt-1">{formik.errors.confirmpassword}</p>
                    )}
                </div>

                <div className="flex flex-col xl:w-4/6 lg:w-4/6 md:w-full sm:w-full mt-6">
                    <label className="font-light text-sm">OTP Code</label>
                    <InputGroup bgColor="#F1EEEE" className="mt-2" size="lg">
                        <InputLeftElement>
                            <FiCode size={25} color="gray" />
                        </InputLeftElement>
                        <Input type="password" name="otp" fontSize="sm" value={formik.values.otp} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                    </InputGroup>
                    {formik.touched.otp && formik.errors.otp && (
                        <p className="text-red-500 font-semibold text-xs mt-1">{formik.errors.otp}</p>
                    )}
                </div>
                ...
                <button disabled={loading} onClick={submit} className="xl:w-4/6 lg:w-4/6 md:w-full sm:w-full mt-8 h-12 bg-themeGreen cursor-pointer flex justify-center items-center">
                    {!loading && <span className="ml-4 text-sm font-semibold text-white">Submit</span>}
