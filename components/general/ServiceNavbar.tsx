...
export default function ServiceNavbar({ search, handleEnter, setSearch}: { search: string, handleEnter: Function, setSearch: Function}) {
    ...
    <div className="w-full h-20 bg-white xl:px-10 lg:px-10 md:px-5 sm:px-5 flex justify-between fixed z-50">
        ...
        <div className="flex-1 h-full xl:hidden lg:hidden md:flex sm:flex items-center px-3">
              <InputGroup>
                  ...
                  <Input type="text" name="search" w="100%" value={search} bgColor="#F1EEEE" paddingLeft="50px"  h="50px" onKeyPress={(e) => handleEnter(e)} onChange={(e) => setSearch(e.target.value)} fontSize="sm" className="font-Cerebri-sans-book" />
